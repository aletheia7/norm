...
{
    unsigned int rxSockBufferSize = 6*1024*1024;  // TBD - make this a command-line option?	    
	bool rval = true ;
	...
	else 
    {
	    ...
	    {
		    fprintf(stderr, "normStreamer failed to set input socket buffer size (%u)\n", rxSockBufferSize);
		    rval = false ;
        }
        if (NULL != groupAddr)
        {
...
int main(int argc, char* argv[])
{
    ...
    while (normStreamer.IsRunning())
    {
        ...
        if (maxfd >= 0)
        {
            ...
            switch (result)
            {
                ...
                case 0:
                    ...
                    fprintf(stderr, "normStreamer timeout fdMask:%d ...\n", fdMask);
                    break;
                default:
        ...
        if ((thisTime.tv_sec - lastTime.tv_sec) >= 5)
        {
            if (send)
