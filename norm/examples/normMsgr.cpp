...
NormMsgr::NormMsgr()
 : input_file(stdin), input_fd(fileno(stdin)), input_ready(false), input_needed(false), input_msg(NULL),
   norm_session(NORM_SESSION_INVALID), is_multicast(false), loopback(false), 
   norm_tx_queue_max(16000), norm_tx_queue_count(0), 
   norm_tx_watermark_pending(false), norm_tx_vacancy(true), norm_acking(false), sent_count(0),
   rx_needed(false), output_file(stdout), output_fd(fileno(stdout)), 
...
bool NormMsgr::OpenNormSession(NormInstanceHandle instance, const char* addr, unsigned short port, NormNodeId nodeId)
{
    ...
    fprintf(stderr, "setting rx cache limit to %hu\n", norm_tx_queue_max);
...
bool NormMsgr::EnqueueMessageObject()
{
    ...
    if (norm_acking)
    {
        ...
        else if (!norm_tx_watermark_pending)
        {
